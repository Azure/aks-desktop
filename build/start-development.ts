...
const { npmInvocation, spawn } = require('../packages/headlamp-source/src/lib/npm-command.ts');
...
export async function startDevelopment(launch = spawn): Promise<void> {
  const frontendPort = await findAvailablePort();
  console.log(`Starting Headlamp frontend on http://localhost:${frontendPort}`);

  const env = developmentEnvironment(frontendPort);
  const invocation = npmInvocation(['run', 'dev:services'], process.platform, env);
