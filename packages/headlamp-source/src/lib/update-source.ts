...
const { resolveWithin } = require('./paths.ts');
...
function copyTrackedSource(sourceDir, destination) {
  ...
  run(
    'git',
    ['-c', 'core.autocrlf=false', 'checkout-index', '--all', '--force', `--prefix=${prefix}`],
    sourceDir
  );
  for (const relativeFile of files) {
...
function prepareHeadlampSource(
  ...
  if (sourceIsMaterialized(packageDir, revision)) {
    materializeHeadlampPatch(rootDir, packageDir);
...
function updateHeadlampSource(
  ...
  const oldPatchPath = project.headlampPatch?.path;
  if (
    typeof oldPatchPath !== 'string' ||
    !/^patches\/headlamp-source@[A-Za-z0-9._+-]+\.patch$/.test(oldPatchPath)
  ) {
    throw new Error(`Expected one ${PACKAGE_NAME} aggregate patch`);
  }
  const newPatchPath = `patches/headlamp-source@${version}.patch`;
  const absoluteOldPatch = resolveWithin(rootDir, oldPatchPath, 'Previous aggregate patch');
  const absoluteNewPatch = resolveWithin(rootDir, newPatchPath, 'New aggregate patch');
  ...
  project.devDependencies[PACKAGE_NAME] = 'file:packages/headlamp-source';
  project.headlampPatch = {
    path: newPatchPath,
    integrity: sha512(absoluteNewPatch),
  };

  lock.packages[''].devDependencies[PACKAGE_NAME] = 'file:packages/headlamp-source';
