 ...
 * @returns `applied` or `unapplied` when all file states agree.
 */
function patchApplicationState(packageDir, aggregate) {
  ...
  for (const entry of entries) {
    ...
    if (currentId === entry.oldId) {
      ...
      throw new Error(`Installed package has unexpected contents: ${entry.relativeFile}`);
    }
  }
  if (oldFiles && newFiles) {
    throw new Error('Installed package contains a partially applied Headlamp patch');
  }
...
function applyHeadlampPatch(
  ...
  try {
    ...
    runGit(['apply', '--whitespace=nowarn', absolutePatch], {
