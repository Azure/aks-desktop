...
test('dereferences source symlinks for relocatable bundles', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'headlamp-copy-links-'));
  const source = path.join(root, 'source');
  const destination = path.join(root, 'destination');
  fs.mkdirSync(path.join(source, 'bin'), { recursive: true });
  fs.writeFileSync(path.join(source, 'bin', 'python3.10'), 'python');
  fs.symlinkSync(path.join(source, 'bin', 'python3.10'), path.join(source, 'bin', 'python3'));
  try {
    copyDirectoryContents(source, destination, true);
    const copiedPython = path.join(destination, 'bin', 'python3');
    assert.equal(fs.lstatSync(copiedPython).isSymbolicLink(), false);
    assert.equal(fs.readFileSync(copiedPython, 'utf8'), 'python');
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
});
