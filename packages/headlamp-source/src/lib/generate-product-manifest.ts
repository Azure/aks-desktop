...
function createManifest(options: any = {}) {
  ...
  const template = createTemplate(project, platform);

  const platformKey = platform === 'darwin' ? 'mac' : platform === 'win32' ? 'win' : platform;
  const icon = build.icons?.[platformKey];
  if (icon !== undefined) {
    if (typeof icon !== 'string' || !icon) {
      throw new Error(`Product icon for ${platformKey} must be a non-empty project-relative path`);
    }
    const iconPath = resolveWithin(rootDir, icon, 'Product icon');
    if (!fs.statSync(iconPath).isFile()) {
      throw new Error(`Product icon is not a regular file: ${iconPath}`);
    }
    template.platforms = {
      ...template.platforms,
      [platformKey]: {
        ...template.platforms?.[platformKey],
        icon: path.relative(appDir, iconPath).split(path.sep).join('/'),
      },
    };
  }
