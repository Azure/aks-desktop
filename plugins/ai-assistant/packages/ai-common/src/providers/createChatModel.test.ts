...
describe('createLangChainModel — validation errors', () => {
  ...
  it('azure: accepts full deployment endpoints and long trailing slash runs', () => {
    expect(() =>
      createChatModel('azure', {
        apiKey: 'k',
        endpoint: `https://x.azure.com/openai/deployments/model${'/'.repeat(100_000)}`,
...
describe('createLangChainModel — vLLM URL normalisation', () => {
  ...
  it('handles long trailing slash runs for vLLM', () => {
    expect(() =>
      createChatModel('vllm', {
        baseUrl: `http://localhost:8080${'/'.repeat(100_000)}`,
        model: 'llama3',
      })
    ).not.toThrow();
  });
