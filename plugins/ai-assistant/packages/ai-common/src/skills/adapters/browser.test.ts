...
describe('BrowserSkillAdapters', () => {
  it('normalizes long archive path-filter separators', async () => {
    const JSZip = (await import('jszip')).default;
    const archive = new JSZip();
    archive.file('owner-repo-sha/skills/SKILL.md', '# skill');
    archive.file('owner-repo-sha/other/ignored.md', '# ignored');
    const bytes = await archive.generateAsync({ type: 'arraybuffer' });

    const files = await createJSZipExtractor().extractTextFiles(
      bytes,
      `${'/'.repeat(100_000)}skills${'/'.repeat(100_000)}`
    );
...
function mockGitHubTreesAndFiles(filePaths: string[]) {
  ...
  const fetchSpy = vi.spyOn(globalThis, 'fetch').mockImplementation(async input => {
    ...
    if (new URL(urlStr).hostname === 'api.github.com') {
