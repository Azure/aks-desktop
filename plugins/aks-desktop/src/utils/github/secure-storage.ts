 ...
 */
export async function secureStorageSave(key: string, value: string): Promise<boolean> {
 ...
 */
export async function secureStorageLoad(key: string): Promise<string | null> {
  const api = getDesktopApi();
  if (!api) return null;
  try {
 ...
 * Returns true on success, false if unavailable or failed.
 */
export async function secureStorageDelete(key: string): Promise<boolean> {
  const api = getDesktopApi();
  if (!api) return false;
  try {
