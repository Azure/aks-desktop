...
export const checkRepoReadiness = async (
  ...
  try {
    ...
    const [hasSetupWorkflow, hasAgentConfig] = await Promise.all([
      fileExists(octokit, owner, repo, '.github/workflows/copilot-setup-steps.yml', ref),
      fileExists(octokit, owner, repo, '.github/agents/containerization.agent.md', ref),
    ]);

    return {
      hasSetupWorkflow,
      hasAgentConfig,
      copilotAgentEnabled: null,
    };
  } catch (error) {
 ...
 * (available when not installed and the app slug can be discovered).
 */
export const checkAppInstallation = async (
  octokit: Octokit,
  owner: string,
  repo: string,
  options?: { repoId?: number; ownerId?: number }
  ...
  try {
    ...
    // Not installed — use known install URL (with API-discovered slug as override if available)
    const appSlug = data.installations[0]?.app_slug;
    const baseUrl = appSlug
      ? `https://github.com/apps/${appSlug}/installations/new`
      : GITHUB_APP_INSTALL_URL;

    // Pre-select the target repo/owner in the GitHub App install flow
    const params = new URLSearchParams();
    if (options?.ownerId) {
      params.set('suggested_target_id', String(options.ownerId));
    }
    if (options?.repoId) {
      params.append('repository_ids[]', String(options.repoId));
    }
    const qs = params.toString();
    const installUrl = qs ? `${baseUrl}?${qs}` : baseUrl;
