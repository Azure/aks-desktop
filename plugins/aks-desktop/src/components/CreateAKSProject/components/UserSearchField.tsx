...
export interface UserSelection {
  ...
  upn?: string;
  /** Whether the UPN was entered manually rather than returned by directory search. */
  upnManuallyEntered?: boolean;
  /** Whether a separately entered UPN should survive an object-ID edit. */
  preserveExistingUpn?: boolean;
}
...
export const UserSearchField: React.FC<UserSearchFieldProps> = ({
  ...
  const applyTypedValue = useCallback(
    (raw: string) => {
      ...
      onChange(
        typedIsUpn
          ? { objectId: '', upn: typed, displayName: typed, upnManuallyEntered: true }
          : { objectId: typed, preserveExistingUpn: true }
      );
  ...
  const handleInputChange = useCallback(
    (_event: React.SyntheticEvent, newInputValue: string, reason: string) => {
      ...
      if (isEntraObjectId(value) || displayName) {
        onChange({ objectId: '', preserveExistingUpn: true });
      }
  ...
  const hasSearchQuery =
    ...
    !isEntraObjectId(inputValue) &&
    !isUserPrincipalName(inputValue) &&
    !showFallbackMessage;
      ...
      <Autocomplete<UserOption, false, false, true>
        ...
        renderInput={params => (
          <TextField
            ...
            label={label}
            variant="outlined"
            error={error && !hasSearchQuery}
            helperText={hasSearchQuery ? undefined : helperText}
            placeholder={
              showFallbackMessage
      ...
      {searchFailed && (
        <Alert severity="error" sx={{ mt: 1, py: 0 }}>
          <Typography variant="caption">
            {t('Try searching again. If the problem continues, check your Azure sign-in.')}
          </Typography>
        </Alert>
      )}
      {noUsersFound && (
