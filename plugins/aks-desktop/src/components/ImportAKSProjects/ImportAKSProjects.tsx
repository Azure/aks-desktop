...
export default function ImportAKSProjects() {
  ...
  const handleImport = async () => {
    ...
    if (successCount > 0) {
      setSuccess(
        `Successfully merged ${successfulClusters} cluster${successfulClusters > 1 ? 's' : ''} with ${successCount} project${successCount > 1 ? 's' : ''}` +
          (failureCount > 0 ? `. ${failureCount} project${failureCount > 1 ? 's' : ''} failed.` : '.')
      );
    } else {
                      ...
                      >
                        ...
                        <Box sx={{ flex: 1 }}>
                          ...
                          <Typography variant="caption" color="textSecondary">
                            Namespace: {item.namespace.name} • Cluster:{' '}
                            {item.namespace.clusterName} • Resource Group:{' '}
                            {item.namespace.resourceGroup}
                          </Typography>
                        </Box>
                      </Box>
                    ))}
