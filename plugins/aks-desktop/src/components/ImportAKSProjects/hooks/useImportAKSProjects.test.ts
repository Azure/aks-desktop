...
let mockRegisteredClustersReady = true;
...
vi.mock('../../../hooks/useRegisteredClusters', () => ({
  useRegisteredClusters: () => ({
    registeredClusters: mockRegisteredClusters,
    isReady: mockRegisteredClustersReady,
  }),
}));
...
describe('useImportAKSProjects', () => {
  beforeEach(() => {
    ...
    mockRegisteredClustersReady = true;
    ...
    mockGetClusterSettings.mockReturnValue({
      azureRegistration: { subscriptionId: 'sub', resourceGroup: 'rg' },
    });
  ...
  test('handleImportClick waits for registered cluster configuration', () => {
    mockRegisteredClustersReady = false;
    const { result } = renderHook(() => useImportAKSProjects());
  ...
  test('matches registered cluster names case-insensitively', async () => {
    mockRegisteredClusters = new Set(['cl']);
    const { result } = renderHook(() => useImportAKSProjects());
  ...
  test('rejects an already registered cluster from another Azure scope', async () => {
    ...
    expect(result.current.importResults![0].message).toContain(
      'already registered with a different cluster kind or Azure scope'
    );
    expect(mockRegisterAKSCluster).not.toHaveBeenCalled();
  });
  ...
  test('rejects an already registered same-name Arc cluster', async () => {
    ...
    expect(result.current.importResults![0].success).toBe(false);
    expect(result.current.importResults![0].message).toContain(
      'different cluster kind or Azure scope'
    );
  ...
  test('adopts an already registered cluster with unknown Azure scope', async () => {
    mockRegisteredClusters = new Set(['cl']);
    mockGetClusterSettings.mockReturnValue({ allowedNamespaces: ['existing'] });
    const { result } = renderHook(() => useImportAKSProjects());
    ...
    expect(result.current.importResults![0].success).toBe(true);
    expect(mockRegisterAKSCluster).not.toHaveBeenCalled();
    expect(mockSetClusterSettings).toHaveBeenCalledWith('cl', {
  ...
  test('fails import when adopting the Azure scope cannot be persisted', async () => {
    ...
    act(() =>
      result.current.handleImportClick([
        makeSelection(makeNamespace('ns-1', 'cl', 'rg', 'sub', false)),
      ])
    );
    act(() => result.current.handleConversionConfirm());
    await waitFor(() => expect(result.current.importing).toBe(false));
    ...
    expect(result.current.importResults![0].success).toBe(false);
    expect(result.current.importResults![0].message).toBe('storage unavailable');
    expect(mockApplyProjectLabels).not.toHaveBeenCalled();
  });

  test('rejects same-name clusters from different Azure scopes', async () => {
    const { result } = renderHook(() => useImportAKSProjects());

    act(() =>
      result.current.handleImportClick([
        makeSelection(makeNamespace('ns-1', 'shared', 'first-rg', 'first-sub')),
        makeSelection(makeNamespace('ns-2', 'Shared', 'second-rg', 'second-sub')),
      ])
    );
    await waitFor(() => expect(result.current.importing).toBe(false));

    expect(result.current.importResults).toHaveLength(2);
    expect(result.current.importResults!.every(item => !item.success)).toBe(true);
    expect(result.current.importResults![0].message).toContain(
      'same cluster name in different Azure scopes'
    );
    expect(mockRegisterAKSCluster).not.toHaveBeenCalled();
  });
  ...
  test('reports progress while registering a cluster', async () => {
    ...
    act(() => result.current.handleImportClick([makeSelection(makeNamespace('ns-1', 'cl'))]));
    await waitFor(() => expect(result.current.importProgress).toBe('Registering cluster: cl'));

    act(() => resolveRegistration({ success: true, message: '' }));
    await waitFor(() => expect(result.current.importing).toBe(false));
  ...
  test('handleGoToProjects replaces history and reloads', () => {
    ...
    const { result } = renderHook(() => useImportAKSProjects({ reloadPage: reloadMock }));
