...
function ActiveSetupPR({
  ...
  const prNumber = pipelineState.setupPr.number;
  const prUrl = pipelineState.setupPr.url;
  ...
  return (
    ...
    >
      ...
      <Typography variant="body2" sx={{ color: 'text.secondary', mb: 2 }}>
        Review and merge the setup PR to enable the Copilot agent.
      </Typography>
...
function ActiveDeploymentPR({
  pipelineState,
  prPolling,
  onReviewInGitHub,
}: {
  pipelineState: PipelineState;
  prPolling: ReviewAndMergeStepProps['generatedPrPolling'];
  onReviewInGitHub: () => void;
}) {
  const prNumber = pipelineState.generatedPr.number;
  const prUrl = pipelineState.generatedPr.url;
  const merged = prPolling.prStatus?.merged ?? false;
  const isClosed = prPolling.prStatus?.state === 'closed' && !merged;
  ...
  return (
    ...
    >
      ...
      <Typography variant="body2" sx={{ color: 'text.secondary', mb: 2 }}>
        The agent has created the deployment PR, review the generated files and merge to start the
        deployment pipeline.
      </Typography>
    ...
    >
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
        ...
        <Typography variant="body2">
          ...
          Copilot agent has reviewed your repo and generated deployment PR.
        </Typography>
        <Typography variant="body2">
          <Box component="span" sx={{ mr: 1 }}>
            →
          </Box>
          Deployment PR review and merged
        </Typography>
      </Box>
      {onViewDeployment && (
        <Button
...
export function ReviewAndMergeStep({
  ...
  if (agentStatus === 'done')
    completedItems.push('Copilot agent has reviewed your repo and generated deployment PR.');
      ...
      {!isPipelineComplete && (
        <>
          {agentStatus === 'upcoming' && (
            <UpcomingSection
              title="Copilot agent"
              description="After merging, the Copilot agent will analyse a repo and create a deployment PR"
            />
          )}
          {deployPRStatus === 'upcoming' && (
            <UpcomingSection
              title="Deployment PR"
              description="Once agent has created deployment PR, review the generated files and merge to start the deployment PR."
            />
          )}
        </>
      )}
