...
/**
 * MCP package version used in the Copilot setup workflow.
 * !! UPDATE to a stable version before each release. !!
 */
const CONTAINERIZATION_MCP_VERSION = 'latest';
...
- Use \`azure/aks-set-context@v4\` with cluster \`${config.clusterName}\` and resource group \`${config.resourceGroup}\`
- Run: \`kubectl apply -f deploy/kubernetes/ -n ${config.namespace}\`
...
export const generateBranchName = (appName: string): string => {
  ...
  return `aks-desktop/setup-${sanitized || 'app'}-${timestamp}`;
};
