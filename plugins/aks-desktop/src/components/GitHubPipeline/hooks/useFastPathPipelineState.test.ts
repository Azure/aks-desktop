...
function persistFailedFastPathState(lastSuccessfulState: FastPathState['deploymentState']) {
  localStorage.setItem(
    `aks-desktop:fast-path-state:${repoKey}`,
    JSON.stringify({
      __schemaVersion: 1,
...
function persistConfiguredFastPathState(key: string) {
  localStorage.setItem(
    `aks-desktop:fast-path-state:${key}`,
    JSON.stringify({
      __schemaVersion: 1,
...
describe('useFastPathPipelineState', () => {
  ...
  it('emits exactly one started and one succeeded event for a live fast-path run', () => {
    const { result, rerender } = renderHook(() => useFastPathPipelineState(repoKey));
  ...
  it('emits a categorical error when the fast path fails', () => {
    const { result } = renderHook(() => useFastPathPipelineState(repoKey));

    act(() => result.current.setConfig(validConfig));
    act(() => result.current.setFailed('synthetic repository failure'));
    ...
    expect(telemetryMocks.trackError).toHaveBeenLastCalledWith({
      area: 'pipeline',
      errorClass: 'UnknownError',
      phase: 'failed',
    });
    expect(JSON.stringify(telemetryMocks.trackError.mock.calls)).not.toContain(
      'synthetic repository failure'
    );
  });

  it('deduplicates same-batch fast-path failure telemetry synchronously', () => {
    const { result } = renderHook(() => useFastPathPipelineState(repoKey));
    act(() => result.current.setConfig(validConfig));
  ...
  it('deduplicates same-batch fast-path success telemetry synchronously', () => {
    const { result } = renderHook(() => useFastPathPipelineState(repoKey));
    act(() => result.current.setConfig(validConfig));
  ...
  it('emits fast-path retry success exactly once after a live failure', () => {
    const { result } = renderHook(() => useFastPathPipelineState(repoKey));
    act(() => result.current.setConfig(validConfig));
    transitionTo(result, 'PipelineRunning');
  ...
  it('emits a second fast-path retry failure exactly once after a live failure', () => {
    const { result } = renderHook(() => useFastPathPipelineState(repoKey));
    act(() => result.current.setConfig(validConfig));
    transitionTo(result, 'PipelineRunning');
  ...
  it('does not replay telemetry when hydrating or switching to a failed repository', () => {
    const failedState = {
      __schemaVersion: 1,
      deploymentState: 'Failed',
      config: validConfig,
      ...
      withAsyncAgent: false,
      serviceEndpoint: null,
      lastSuccessfulState: 'DockerfileDetected',
    ...
    localStorage.setItem('aks-desktop:fast-path-state:second/repo', JSON.stringify(failedState));

    const { result, rerender } = renderHook(({ key }) => useFastPathPipelineState(key), {
      initialProps: { key: repoKey },
    });
  ...
  it('does not emit started when setting an already persisted configured fast-path config', () => {
    persistConfiguredFastPathState(repoKey);
    const { result } = renderHook(() => useFastPathPipelineState(repoKey));
  ...
  it('does not emit started after loading a configured fast-path state with config', () => {
    const { result, rerender } = renderHook(({ key }) => useFastPathPipelineState(key), {
      initialProps: { key: repoKey },
    });
    persistConfiguredFastPathState('second/repo');
  ...
  it('does not emit started after loadState restores a configured fast-path state with config', () => {
    const { result } = renderHook(() => useFastPathPipelineState(repoKey));
    const configuredState: FastPathState = {
      deploymentState: 'Configured',
      config: validConfig,
      dockerfilePaths: [],
      fastPathPr: { url: null, number: null, merged: false },
    ...
    act(() => result.current.loadState(configuredState));
    act(() => result.current.setConfig(validConfig));

    expect(telemetryMocks.trackFeature).not.toHaveBeenCalled();
  });

  it('emits persisted fast-path retry success exactly once', () => {
    persistFailedFastPathState('PipelineRunning');
    const { result } = renderHook(() => useFastPathPipelineState(repoKey));
  ...
  it('emits persisted fast-path retry failure exactly once', () => {
    persistFailedFastPathState('DockerfileDetected');
    const { result } = renderHook(() => useFastPathPipelineState(repoKey));
  ...
  it('does not replay terminal telemetry through loadState', () => {
    const { result, rerender } = renderHook(() => useFastPathPipelineState(repoKey));
    const failedState: FastPathState = {
      deploymentState: 'Failed',
      config: validConfig,
      dockerfilePaths: ['Dockerfile'],
      fastPathPr: { url: null, number: null, merged: false },
    ...
    act(() => result.current.loadState(failedState));
    rerender();
