...
describe('useGitHubPipelineState', () => {
  ...
  describe('setRepoReadiness', () => {
    ...
    it('should transition to CopilotNotEnabled when agent is disabled', () => {
      const { result } = renderHook(() => useGitHubPipelineState(null));
      ...
      expect(result.current.state.deploymentState).toBe('CopilotNotEnabled');
    });
  });
