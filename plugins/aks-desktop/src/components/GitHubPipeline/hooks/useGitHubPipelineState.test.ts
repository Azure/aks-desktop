...
const persistedRepoKey = 'testuser/my-repo';

function persistFailedPipelineState(lastSuccessfulState: PipelineState['deploymentState']) {
  localStorage.setItem(
    `aks-desktop:pipeline-state:${persistedRepoKey}`,
    JSON.stringify({
      __schemaVersion: 1,
...
function persistConfiguredPipelineState(key: string) {
  localStorage.setItem(
    `aks-desktop:pipeline-state:${key}`,
    JSON.stringify({
      __schemaVersion: 1,
...
describe('useGitHubPipelineState', () => {
  beforeEach(() => {
    telemetryMocks.trackFeature.mockReset();
    telemetryMocks.trackError.mockReset();
    localStorage.clear();
  });

  it('emits exactly one started and one completed event for a live configure run', () => {
    const { result, rerender } = renderHook(() => useGitHubPipelineState(null));

    act(() => result.current.setConfig(validConfig));
    transitionTo(result, 'PipelineConfigured');
    rerender();

    expect(telemetryMocks.trackFeature.mock.calls).toEqual([
      [{ feature: 'aksd.pipeline', status: 'started' }],
      [{ feature: 'aksd.pipeline', status: 'completed' }],
    ]);
    expect(telemetryMocks.trackError).not.toHaveBeenCalled();
  });

  it('emits exactly one succeeded event for a live deployment transition', () => {
    const { result, rerender } = renderHook(() => useGitHubPipelineState(null));
    ...
    expect(telemetryMocks.trackFeature.mock.calls).toEqual([
      [{ feature: 'aksd.pipeline', status: 'started' }],
      [{ feature: 'aksd.pipeline', status: 'succeeded' }],
    ]);
    expect(telemetryMocks.trackError).not.toHaveBeenCalled();
  });

  it('reports a categorical pipeline failure without forwarding its message', () => {
    const { result } = renderHook(() => useGitHubPipelineState(null));
    const sensitiveMessage = 'failure for synthetic/repository at https://example.test';

    act(() => result.current.setFailed(sensitiveMessage));

    expect(telemetryMocks.trackFeature).toHaveBeenCalledTimes(1);
    expect(telemetryMocks.trackFeature).toHaveBeenCalledWith({
      feature: 'aksd.pipeline',
      status: 'failed',
    });
    ...
    expect(JSON.stringify(telemetryMocks.trackError.mock.calls)).not.toContain(sensitiveMessage);
  });

  it('deduplicates same-batch failure telemetry synchronously', () => {
    const { result } = renderHook(() => useGitHubPipelineState(null));

    act(() => {
      result.current.setFailed('first failure');
      result.current.setFailed('second failure');
    });

    expect(telemetryMocks.trackFeature.mock.calls).toEqual([
      [{ feature: 'aksd.pipeline', status: 'failed' }],
    ]);
    expect(telemetryMocks.trackError.mock.calls).toEqual([
      [{ area: 'pipeline', errorClass: 'UnknownError', phase: 'failed' }],
    ]);
  });

  it('deduplicates same-batch deployment success telemetry synchronously', () => {
    const { result } = renderHook(() => useGitHubPipelineState(null));
    transitionTo(result, 'PipelineRunning');
    telemetryMocks.trackFeature.mockClear();
    telemetryMocks.trackError.mockClear();

    act(() => {
      result.current.setDeployed();
      result.current.setDeployed();
    });
  ...
  it('deduplicates same-batch pipeline completion telemetry synchronously', () => {
    const { result } = renderHook(() => useGitHubPipelineState(null));
    ...
    act(() => {
      result.current.setPipelineConfigured();
      result.current.setPipelineConfigured();
    });
  ...
  it('emits retry success exactly once after a live failure', () => {
    const { result } = renderHook(() => useGitHubPipelineState(null));
    transitionTo(result, 'PipelineRunning');

    act(() => result.current.setFailed('first attempt failed'));
  ...
  it('emits a second retry failure exactly once after a live failure', () => {
    const { result } = renderHook(() => useGitHubPipelineState(null));
    transitionTo(result, 'PipelineRunning');
    ...
    act(() => result.current.retry());
    act(() => {
      result.current.setFailed('retry failed');
      result.current.setFailed('duplicate retry failure');
    });
  ...
  it('does not replay failure telemetry when hydrating a failed pipeline', () => {
    localStorage.setItem(
      ...
      JSON.stringify({
        ...
        generatedPr: { url: null, number: null, merged: false },
        serviceEndpoint: null,
        lastSuccessfulState: 'ReadyForSetup',
    ...
    const { result, rerender } = renderHook(() => useGitHubPipelineState('testuser/my-repo'));
    rerender();
    act(() => result.current.setFailed('replayed failure'));

    expect(result.current.state.deploymentState).toBe('Failed');
    expect(telemetryMocks.trackFeature).not.toHaveBeenCalled();
    expect(telemetryMocks.trackError).not.toHaveBeenCalled();
  });

  it('does not emit started when setting an already persisted configured pipeline config', () => {
    persistConfiguredPipelineState(persistedRepoKey);
    const { result } = renderHook(() => useGitHubPipelineState(persistedRepoKey));
  ...
  it('does not emit started after loading a configured pipeline state with config', () => {
    const { result, rerender } = renderHook(({ key }) => useGitHubPipelineState(key), {
      initialProps: { key: 'first/repo' },
    });
    persistConfiguredPipelineState('second/repo');

    rerender({ key: 'second/repo' });
    act(() => result.current.setConfig(validConfig));

    expect(telemetryMocks.trackFeature).not.toHaveBeenCalled();
  });

  it('emits persisted retry deployment success exactly once', () => {
    persistFailedPipelineState('PipelineRunning');
    const { result } = renderHook(() => useGitHubPipelineState(persistedRepoKey));
  ...
  it('emits persisted retry failure exactly once', () => {
    persistFailedPipelineState('ReadyForSetup');
    const { result } = renderHook(() => useGitHubPipelineState(persistedRepoKey));
  ...
  it('emits persisted retry pipeline completion exactly once', () => {
    persistFailedPipelineState('GeneratedPRAwaitingMerge');
    const { result } = renderHook(() => useGitHubPipelineState(persistedRepoKey));
  ...
  it('does not replay terminal telemetry when switching repositories', () => {
    localStorage.setItem(
      'aks-desktop:pipeline-state:second/repo',
      JSON.stringify({
        __schemaVersion: 1,
        deploymentState: 'Failed',
        config: validConfig,
  ...
  it('resets terminal deduplication only when a fresh repository starts a live run', () => {
    const { result, rerender } = renderHook(({ key }) => useGitHubPipelineState(key), {
      initialProps: { key: 'first/repo' },
    });
    act(() => result.current.setFailed('first run failed'));
    telemetryMocks.trackFeature.mockClear();
    telemetryMocks.trackError.mockClear();

    rerender({ key: 'fresh/repo' });
    act(() => {
      result.current.setConfig(validConfig);
      result.current.setFailed('fresh run failed');
      result.current.setFailed('duplicate fresh failure');
    });
  ...
  it('telemetry failures do not block pipeline state transitions', () => {
