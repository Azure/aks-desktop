...
export function usePrometheusMetrics(
  ...
  const fetchMetrics = useCallback(async () => {
    ...
    try {
      ...
      // Query CPU usage
      const cpuQuery = `sum by (namespace) (rate(container_cpu_usage_seconds_total{namespace="${namespace}", pod=~"${selectedDeployment}-.*", container!=""}[5m]))`;
      ...
      const cpuByPodQuery = `sum by (pod) (rate(container_cpu_usage_seconds_total{namespace="${namespace}", pod=~"${selectedDeployment}-.*", container!=""}[5m]))`;
      ...
      const memoryQuery = `sum by (namespace) (container_memory_working_set_bytes{namespace="${namespace}", pod=~"${selectedDeployment}-.*", container!=""})`;
      ...
      const memoryByPodQuery = `sum by (pod) (container_memory_working_set_bytes{namespace="${namespace}", pod=~"${selectedDeployment}-.*", container!=""})`;
      ...
      const networkInQuery = `sum by (namespace) (rate(container_network_receive_bytes_total{namespace="${namespace}", pod=~"${selectedDeployment}-.*"}[5m]))`;
      const networkOutQuery = `sum by (namespace) (rate(container_network_transmit_bytes_total{namespace="${namespace}", pod=~"${selectedDeployment}-.*"}[5m]))`;
